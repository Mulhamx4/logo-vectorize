...
منشورة باسم [`logo-vectorizer`](https://www.npmjs.com/package/logo-vectorizer) وتحتوي `site/lib/` فقط:

```bash
npm install logo-vectorizer
```

الإصدار 1.1.0 ثُبّت من سجل npm داخل مشروع Vite 8.3 + React 19.3 + TypeScript 6.0.3: نجح `tsc --noEmit` و`vite build` وخادم التطوير، واشتغل تتبع عدة شعارات وتصدير PDF وزر المضيف.
...
- **انسخ `site/lib/` كاملًا بدون تعديل** (أو ثبّت الحزمة من npm). التحديث = إعادة نسخ المجلد من المستودع. أي تعديل داخله يضيع مع أول تحديث.
